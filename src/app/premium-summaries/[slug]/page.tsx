...
export default function PremiumSummaryDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        setIsLoading(true);
        const response = await fetch(`${API_URL}/premium-summaries`);
        ...
        console.error('Error fetching premium summary:', err);
        setSummary(null);
      } finally {
        setIsLoading(false);
      }
      ...
      <div className="bg-white border-b sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          >
            ...
            <span className="font-medium">Back to Premium Summaries</span>
          </button>
        </div>
      </div>
